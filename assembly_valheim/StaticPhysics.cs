...
public class StaticPhysics : SlowUpdate
{
	...
	private float GetFallHeight()
	{
		...
		{
			float num;
			if (ZoneSystem.instance.GetSolidHeight(base.transform.position, this.m_fallCheckRadius, out num, base.transform))
			{
				return num;
			}
		...
		{
			float num2;
			if (ZoneSystem.instance.GetGroundHeight(base.transform.position, out num2))
			{
				return num2;
			}
			return base.transform.position.y;
