...
public class Plant : SlowUpdate, Hoverable
{
	...
	{
		DateTime dateTime = new DateTime(this.m_nview.GetZDO().GetLong(ZDOVars.s_plantTime, ZNet.instance.GetTime().Ticks));
	...
	private void Grow()
	{
		...
		GameObject gameObject = this.m_grownPrefabs[UnityEngine.Random.Range(0, this.m_grownPrefabs.Length)];
		Quaternion quaternion = Quaternion.Euler(0f, UnityEngine.Random.Range(0f, 360f), 0f);
		GameObject gameObject2 = UnityEngine.Object.Instantiate<GameObject>(gameObject, base.transform.position, quaternion);
		ZNetView component = gameObject2.GetComponent<ZNetView>();
		float num = UnityEngine.Random.Range(this.m_minScale, this.m_maxScale);
		component.SetLocalScale(new Vector3(num, num, num));
		TreeBase component2 = gameObject2.GetComponent<TreeBase>();
		if (component2)
		{
			component2.Grow();
		}
	...
	private bool HaveRoof()
	{
		...
		{
			Plant.m_roofMask = LayerMask.GetMask(new string[] { "Default", "static_solid", "piece" });
		}
	...
	private bool HaveGrowSpace()
	{
		...
		{
			Plant.m_spaceMask = LayerMask.GetMask(new string[] { "Default", "static_solid", "Default_small", "piece", "piece_nonsolid" });
		}
