...
public class ZBroastcast : IDisposable
{
	...
	private void GotPackage(IAsyncResult ar)
	{
		...
		this.m_lock.WaitOne();
		this.AddHost(ipendPoint.Address.ToString(), num);
	...
	private void Ping()
	{
		IPEndPoint ipendPoint = new IPEndPoint(IPAddress.Broadcast, 6542);
		...
		this.m_socket.SendTo(zpackage.GetArray(), ipendPoint);
	}
