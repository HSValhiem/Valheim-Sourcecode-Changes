...
public class Windmill : MonoBehaviour
{
	...
	private void Update()
	{
		...
		this.m_bom.rotation = Quaternion.RotateTowards(this.m_bom.rotation, quaternion, this.m_bomRotationSpeed * powerOutput * Time.deltaTime);
	...
	{
		float powerOutput = this.GetPowerOutput();
		float num = Mathf.Lerp(this.m_minPitch, this.m_maxPitch, Mathf.Clamp01(powerOutput / this.m_maxPitchVel));
		float num2 = this.m_maxVol * Mathf.Clamp01(powerOutput / this.m_maxVolVel);
		foreach (AudioSource audioSource in this.m_sfxLoops)
		{
			audioSource.volume = Mathf.MoveTowards(audioSource.volume, num2, this.m_audioChangeSpeed * dt);
			audioSource.pitch = Mathf.MoveTowards(audioSource.pitch, num, this.m_audioChangeSpeed * dt);
		}
	}
