...
public class Valkyrie : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		Vector3 vector3 = this.m_targetPoint + vector * this.m_startDistance;
		vector3.y = this.m_startAltitude;
		base.transform.position = vector3;
		this.m_descentStart = this.m_targetPoint + vector * this.m_startDescentDistance + vector2 * 200f;
		this.m_descentStart.y = this.m_descentAltitude;
		Vector3 vector4 = this.m_targetPoint - this.m_descentStart;
		vector4.y = 0f;
		vector4.Normalize();
		this.m_flyAwayPoint = this.m_targetPoint + vector4 * this.m_startDescentDistance;
	...
	private void UpdateValkyrie(float dt)
	{
		...
		Vector3 vector3 = normalized2;
		vector3.y = 0f;
		vector3.Normalize();
		Vector3 forward = base.transform.forward;
		forward.y = 0f;
		forward.Normalize();
		float num2 = Mathf.Clamp(Vector3.SignedAngle(forward, vector3, Vector3.up), -30f, 30f) / 30f;
		...
		Vector3 vector4 = base.transform.forward * this.m_speed;
		Vector3 vector5 = base.transform.position + vector4 * dt;
		float num4;
		if (ZoneSystem.instance.GetGroundHeight(vector5, out num4))
		{
			vector5.y = Mathf.Max(vector5.y, num4 + this.m_dropHeight);
		}
