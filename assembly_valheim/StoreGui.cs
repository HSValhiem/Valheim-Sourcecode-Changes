...
public class StoreGui : MonoBehaviour
{
	...
	private void BuySelectedItem()
	{
		...
		int num = Mathf.Min(this.m_selectedItem.m_stack, this.m_selectedItem.m_prefab.m_itemData.m_shared.m_maxStackSize);
		...
		if (Player.m_localPlayer.GetInventory().AddItem(this.m_selectedItem.m_prefab.name, num, quality, variant, 0L, "") != null)
		{
	...
	private void SellItem()
	{
		...
		int num = sellableItem.m_shared.m_value * sellableItem.m_stack;
		Player.m_localPlayer.GetInventory().RemoveItem(sellableItem);
		Player.m_localPlayer.GetInventory().AddItem(this.m_coinPrefab.gameObject.name, num, this.m_coinPrefab.m_itemData.m_quality, this.m_coinPrefab.m_itemData.m_variant, 0L, "");
		string text;
	...
	private void SelectItem(int index, bool center)
	{
		...
		{
			bool flag = i == index;
			this.m_itemList[i].transform.Find("selected").gameObject.SetActive(flag);
		}
		if (center && index >= 0)
		{
