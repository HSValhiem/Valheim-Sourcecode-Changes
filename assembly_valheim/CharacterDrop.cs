...
public class CharacterDrop : MonoBehaviour
{
	...
	private void OnDeath()
	{
		...
		List<KeyValuePair<GameObject, int>> list = this.GenerateDropList();
		Vector3 vector = this.m_character.GetCenterPoint() + base.transform.TransformVector(this.m_spawnOffset);
		CharacterDrop.DropItems(list, vector, 0.5f);
	}
	...
	public static void DropItems(List<KeyValuePair<GameObject, int>> drops, Vector3 centerPos, float dropArea)
	{
		foreach (KeyValuePair<GameObject, int> keyValuePair in drops)
		{
			for (int i = 0; i < keyValuePair.Value; i++)
			{
				...
				GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(keyValuePair.Key, centerPos + vector, quaternion);
