...
public class ZConnector : IDisposable
{
	...
	private void Close()
	{
		if (this.m_socket != null)
		{
			...
			{
				string text = "Some excepetion when shuting down ZConnector socket, ignoring:";
				Exception ex2 = ex;
	...
	public ZSocket Complete()
	{
		...
		{
			ZSocket zsocket = new ZSocket(this.m_socket, this.m_host);
			this.m_socket = null;
			return zsocket;
		}
		this.Close();
	...
	private void OnHostLookupDone(IAsyncResult res)
	{
		...
		foreach (IPAddress ipaddress in iphostEntry.AddressList)
		{
			...
			ZLog.Log(text + ((ipaddress2 != null) ? ipaddress2.ToString() : null));
		}
