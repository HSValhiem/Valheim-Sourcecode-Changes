...
public class Thunder : MonoBehaviour
{
	...
	{
		float num = UnityEngine.Random.value * 6.28318548f;
		...
		float num2 = num + 180f + (float)UnityEngine.Random.Range(-45, 45);
		Vector3 vector2 = base.transform.position + new Vector3(Mathf.Sin(num2), 0f, Mathf.Cos(num2)) * this.m_thorSpawnDistance;
	...
	{
		float num = UnityEngine.Random.value * 6.28318548f;
		float num2 = UnityEngine.Random.Range(this.m_flashDistanceMin, this.m_flashDistanceMax);
		this.m_flashPos = base.transform.position + new Vector3(Mathf.Sin(num), 0f, Mathf.Cos(num)) * num2;
		this.m_flashPos.y = this.m_flashPos.y + this.m_flashAltitude;
		Quaternion quaternion = Quaternion.LookRotation((base.transform.position - this.m_flashPos).normalized);
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				componentsInChildren[j].transform.rotation = quaternion;
			}
		}
