...
public class Container : MonoBehaviour, Hoverable, Interactable
{
	...
	{
		foreach (ItemDrop.ItemData itemData in this.m_defaultItems.GetDropListItems())
		{
			this.m_inventory.AddItem(itemData);
		}
	}
	...
	private void DropAllItems(GameObject lootContainerPrefab)
	{
		...
		{
			Vector3 vector = base.transform.position + UnityEngine.Random.insideUnitSphere * 1f;
			UnityEngine.Object.Instantiate<GameObject>(lootContainerPrefab, vector, UnityEngine.Random.rotation).GetComponent<Container>().GetInventory()
				.MoveAll(this.m_inventory);
		}
	}
	...
	private void DropAllItems()
	{
		...
		{
			Vector3 vector = base.transform.position + Vector3.up * 0.5f + UnityEngine.Random.insideUnitSphere * 0.3f;
			Quaternion quaternion = Quaternion.Euler(0f, (float)UnityEngine.Random.Range(0, 360), 0f);
			ItemDrop.DropItem(itemData, 0, vector, quaternion);
			num++;
		}
	...
	public bool Interact(Humanoid character, bool hold, bool alt)
	{
		...
		this.m_nview.InvokeRPC("RequestOpen", new object[] { playerID });
		return true;
	}
	...
	private void RPC_RequestOpen(long uid, long playerID)
	{
		...
		{
			ZLog.Log("  in use");
			this.m_nview.InvokeRPC(uid, "OpenRespons", new object[] { false });
			return;
		}
		if (!this.CheckAccess(playerID))
		{
		...
		this.m_nview.InvokeRPC(uid, "OpenRespons", new object[] { true });
	}
	...
	public bool TakeAll(Humanoid character)
	{
		...
		this.m_nview.InvokeRPC("RequestTakeAll", new object[] { playerID });
		return true;
	}
	...
	private void RPC_RequestTakeAll(long uid, long playerID)
	{
		...
		{
			ZLog.Log("  in use");
			this.m_nview.InvokeRPC(uid, "TakeAllRespons", new object[] { false });
			return;
		}
		if (!this.CheckAccess(playerID))
		{
		...
		this.m_nview.InvokeRPC(uid, "TakeAllRespons", new object[] { true });
	}
	...
	private void Load()
	{
		...
		this.m_loading = true;
		this.m_inventory.Load(zpackage);
