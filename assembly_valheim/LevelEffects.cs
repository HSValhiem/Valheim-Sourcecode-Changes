...
public class LevelEffects : MonoBehaviour
{
	...
	private void SetupLevelVisualization(int level)
	{
		...
		if (this.m_levelSetups.Count >= level - 1)
		{
			...
			{
				string text = Utils.GetPrefabName(this.m_character.gameObject) + level.ToString();
				Material material;
				if (LevelEffects.m_materials.TryGetValue(text, out material))
				{
				...
				else
				{
					...
					LevelEffects.m_materials[text] = sharedMaterials2[0];
				}
			}
