...
public class PointGenerator
{
	...
	private void RegeneratePoints(Vector2Int centerGrid, int gridWith)
	{
		...
		for (int i = centerGrid.y - gridWith; i <= centerGrid.y + gridWith; i++)
		{
			for (int j = centerGrid.x - gridWith; j <= centerGrid.x + gridWith; j++)
			{
				...
				{
					Vector3 vector = new Vector3(UnityEngine.Random.Range(gridPos.x - this.m_gridSize, gridPos.x + this.m_gridSize), UnityEngine.Random.Range(gridPos.z - this.m_gridSize, gridPos.z + this.m_gridSize));
					this.m_points.Add(vector);
				}
			}
		}
	...
	{
		int num = Mathf.FloorToInt((point.x + this.m_gridSize / 2f) / this.m_gridSize);
		int num2 = Mathf.FloorToInt((point.z + this.m_gridSize / 2f) / this.m_gridSize);
