...
public class ImpactEffect : MonoBehaviour
{
	...
	public void OnCollisionEnter(Collision info)
	{
		...
		Vector3 vector = Vector3.Cross(-Vector3.Normalize(info.relativeVelocity), contactPoint.normal);
		Vector3 vector2 = Vector3.Cross(contactPoint.normal, vector);
		Quaternion quaternion = Quaternion.identity;
		if (vector2 != Vector3.zero && contactPoint.normal != Vector3.zero)
		{
			quaternion = Quaternion.LookRotation(vector2, contactPoint.normal);
		}
		this.m_hitEffect.Create(point, quaternion, null, 1f, -1);
		if (this.m_firstHit && this.m_hitDestroyChance > 0f && UnityEngine.Random.value <= this.m_hitDestroyChance)
		{
			this.m_destroyEffect.Create(point, quaternion, null, 1f, -1);
